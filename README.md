...
FastAPI `BackgroundTasks` is also not a durable queue: work can be lost on process restart and is not coordinated across multiple workers. Replace it with a durable queue before production/multi-worker deployment.
